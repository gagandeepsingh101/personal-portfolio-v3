...
const SideProfile: React.FC = () => {
    ...
    return (
        <div className={'w-11/12 my-4 relative flex flex-col  justify-evenly items-center px-2 py-5 md:p-5 bg-lightBlack border border-[#ffffff24] rounded-xl lg:w-[20%] transition-all duration-300 ease-linear overflow-hidden lg:h-full ' + (showMoreInfo ? "h-[80vh] md:h-[65vh]" : " h-[25vh] md:h-[30vh]")} >
            ...
            {showMoreInfo && (<>
                ...
                <div className={' flex flex-col w-full  flex-wrap md:flex-row lg:flex-col transition-all duration-300 ease-linear ' + (showMoreInfo ? "opacity-100" : "opacity-0")}>
                    {InfoSectionArray.map((info,index) => (<InfoSection key={index} iconName={info.iconName} iconText={info.iconText} iconImage={info.iconImage}></InfoSection>))}
                </div>
